...
import { connect,Dispatch, IndexModelState, ConnectProps,useDispatch } from 'umi';
...
const IndexPage: FC<PageProps> = (indexProps ) => {
  let {index:{heros},dispatch}=indexProps
  const onChange = () => {
    dispatch({
  ...
  return (
    <div>
      ...
      {
        heros.map((item)=>{
          return  <span className={styles.title} key={item.ename}>{item.cname}</span>
        })
      }
    </div>
  );
